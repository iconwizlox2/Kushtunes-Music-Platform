...
import { parseJsonOrForm, updateAdvanceSchema } from "@/lib/validators";
...
export async function PATCH(req: Request, ctx: { params: { id: string } }) {
  try {
    await requireAdmin();
    const id = ctx.params.id;
    const body = await parseJsonOrForm(req, updateAdvanceSchema);
    const adv = await prisma.advance.update({
      where: { id },
      data: {
